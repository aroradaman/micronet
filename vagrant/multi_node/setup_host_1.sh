...
ip netns exec ${netns_1} ip addr add ${host_1_ip_pod_1}/24 dev eth0
## add ip to "eth0" interface of pod2
ip netns exec ${netns_2} ip addr add ${host_1_ip_pod_2}/24 dev eth0
...
ip addr add ${host_1_ip_bridge}/24 dev br0
...
ip route add ${host_1_pod_cidr} dev br0
ip route add ${host_2_pod_cidr} via ${host_2_ip} dev ${main_interface}
ip route add ${vagrant_cidr} dev ${main_interface}
## on pod1
ip netns exec ${netns_1} ip route add default via ${host_1_ip_bridge} dev eth0
### the following route might be created by system itself after enabling eth0 device in pod1 netns
ip netns exec ${netns_1} ip route add ${host_1_pod_cidr} dev eth0
## on pod2
ip netns exec ${netns_2} ip route add default via ${host_1_ip_bridge} dev eth0
### the following route might be created by system itself after enabling eth0 device in pod2 netns
ip netns exec ${netns_2} ip route add ${host_1_pod_cidr} dev eth0
